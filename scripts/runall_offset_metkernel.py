...
# %%
CACHE_DIR = Path("cache_offset_metrics_kernel")
...
# %%
def cosine_gram(df: pd.DataFrame, standardize: bool = True) -> pd.DataFrame:
    """
    df: rows=modules, cols=metrics
...
# %%
# cosine kernel from metrics
gram_key = make_cache_key(
    "metrics_cosine_gram",
    list(metx.index),
    list(metx.columns),
    metx.shape,
...
if df_gram is None:
    df_gram = cosine_gram(metx, standardize=True)
...
print("metrics dim:", metx.shape[1], "gram dim:", df_gram.shape[1])
