...
# %%
CACHE_DIR = Path("cache_offset_embpca")
...
# %%
# Build truncated datasets aligned to each t_end (same index i as t_end[i])
# faultdata_by_t[i] corresponds to truncation at t_end[i]
...
pca_key = make_cache_key("pca95", list(df.index), df.shape, "n_components=0.95")
p = cache_path(f"df_pca_{pca_key}")
...
print("embedding dim:", df.shape[1], "pca dim:", df_pca.shape[1])
