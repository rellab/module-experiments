...
# %%
CACHE_DIR = Path("cache_offset_embkernel")
...
def pred_err(models: dict, faultdata0: dict, faultdata1: dict) -> float:
    # Treat the prediction interval as the increment over [t0, t1].
    err = 0.0
...
# %%
def cosine_gram(df: pd.DataFrame) -> pd.DataFrame:
    """
    df: rows=modules, cols=embedding dimensions
    return: cosine similarity Gram matrix
    """
    K = cosine_similarity(df.values)
    K = pd.DataFrame(K, index=df.index, columns=df.index)
    return K
...
# %%
# cosine kernel (embedding vector -> Gram matrix)
gram_key = make_cache_key("cosine_gram", list(df.index), df.shape)
p = cache_path(f"df_gram_{gram_key}")
df_gram = joblib_load_if_exists(p)

if df_gram is None:
    df_gram = cosine_gram(df)
    df_gram = df_gram.loc[module_names, module_names]
    joblib_dump(df_gram, p)

# Convert to SMetricsData.
# Use the similarity vector to other modules as the feature representation for each module.
smetdat = SMetricsData.from_dataframe(df_gram, use_index_as_name=True)

print("modules:", len(target_modules))
print("embedding dim:", df.shape[1], "gram dim:", df_gram.shape[1])
