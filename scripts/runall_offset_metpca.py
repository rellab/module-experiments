...
# %%
CACHE_DIR = Path("cache_offset_metpca95")
...
cache_key = make_cache_key("faultdata_by_t_metrics", sorted(target_modules), t_end)
p = cache_path(f"faultdata_by_t_metrics_{cache_key}")
...
# %%
# PCA (95% explained variance) -> SMetricsData (cache)
...
if cached is None:
    X = StandardScaler().fit_transform(X_metrics.values)
...
df_pca = cached["df_pca"]
smetdat = SMetricsData.from_dataframe(df_pca, use_index_as_name=True)

print("modules:", len(module_names))
print("metrics dim:", X_metrics.shape[1], "pca dim:", df_pca.shape[1])
print("cum explained variance:", cached["cum_explained_variance"][-1])
...
for i in range(len(t_end) - 1):
    ...
    key = make_cache_key("baseline_metrics", train_t, test_t, sorted(target_modules))
    p = cache_path(f"baseline_metrics_{key}")
...
for i in range(len(t_end) - 1):
    ...
    p = cache_path(f"lambda_sweep_metrics_{step_key}")
