...
# %%
CACHE_DIR = Path("cache_offset_metraw")
...
# %%
# load / align
# Use the embedding-side index to align with `faultdata`.
df_embed = load_embedding_dataframe("group_embeddings.jsonl")
met = pd.read_csv("module_metrics.csv", index_col=0)

diff = met.index.symmetric_difference(df_embed.index)
df_embed = df_embed.drop(diff, errors="ignore")
met = met.drop(diff, errors="ignore")
assert set(df_embed.index) == set(met.index), "Mismatch in folders after alignment"

faultdata_full = load_all_data(df_embed)
...
cache_key = make_cache_key("faultdata_by_t_metrics_raw", sorted(target_modules), t_end)
p = cache_path(f"faultdata_by_t_metrics_raw_{cache_key}")
...
metric_cols_exclude = {"sum_nloc"}
X_metrics = met.drop(columns=[c for c in metric_cols_exclude if c in met.columns]).copy()

# Use numeric columns only.
X_metrics = X_metrics.select_dtypes(include=[np.number]).copy()

# Handle missing and infinite values.
X_metrics = X_metrics.replace([np.inf, -np.inf], np.nan)
na_cols = X_metrics.columns[X_metrics.isna().any()].tolist()
if na_cols:
    print("Dropping columns with NaN/inf:", na_cols)
    X_metrics = X_metrics.drop(columns=na_cols)

# Drop constant columns.
nunique = X_metrics.nunique(dropna=False)
const_cols = nunique[nunique <= 1].index.tolist()
if const_cols:
    print("Dropping constant columns:", const_cols)
    X_metrics = X_metrics.drop(columns=const_cols)

if X_metrics.shape[1] == 0:
    raise ValueError("No usable metric columns after filtering.")

print("metric columns used:", list(X_metrics.columns))
print("n metric columns:", X_metrics.shape[1])

# %%
# raw metrics (no PCA) -> SMetricsData (cache)
raw_key = make_cache_key(
    "metrics_raw",
    list(X_metrics.index),
    list(X_metrics.columns),
    X_metrics.shape,
    "exclude=sum_nloc",
    "scale=True",
)
p = cache_path(f"metrics_raw_{raw_key}")
cached = joblib_load_if_exists(p)

if cached is None:
    # Use raw metrics as-is, but still normalize their scale.
    X_scaled = StandardScaler().fit_transform(X_metrics.values)

    df_metrics_raw = pd.DataFrame(
        X_scaled,
        index=X_metrics.index,
        columns=X_metrics.columns,
    )

    cached = {
        "df_metrics_raw": df_metrics_raw,
        "n_components": df_metrics_raw.shape[1],
        "used_metric_columns": list(X_metrics.columns),
    }
    joblib_dump(cached, p)

df_metrics_raw = cached["df_metrics_raw"]
smetdat = SMetricsData.from_dataframe(df_metrics_raw, use_index_as_name=True)

print("modules:", len(module_names))
print("metrics dim:", X_metrics.shape[1])
print("used raw metric columns:", cached["used_metric_columns"])
...
for i in range(len(t_end) - 1):
    ...
    key = make_cache_key("baseline_metrics_raw", train_t, test_t, sorted(target_modules))
    p = cache_path(f"baseline_metrics_raw_{key}")
...
for i in range(len(t_end) - 1):
    ...
    step_key = make_cache_key(
        ...
        "metrics_raw_scaled",
        tuple(X_metrics.columns),
    )
    p = cache_path(f"lambda_sweep_metrics_raw_{step_key}")
