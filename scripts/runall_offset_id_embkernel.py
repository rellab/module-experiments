...
# %%
CACHE_DIR = Path("cache_offset_identity_smetrics_embprecision_l2_emb12")
...
def cosine_kernel(df: pd.DataFrame) -> pd.DataFrame:
    """
    df: rows=modules, cols=embedding dimensions
    return: cosine similarity kernel matrix
    """
    K = cosine_similarity(df.values)
    return pd.DataFrame(K, index=df.index, columns=df.index)
...
# %%
# l2matrix = inverse(cosine kernel + jitter I)
kernel_name = "cosine"
kernel_jitter = 1e-6
...
if df_l2matrix is None:
    df_l2matrix = cosine_precision(df, jitter=kernel_jitter)
