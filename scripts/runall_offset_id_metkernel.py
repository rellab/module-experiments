...
# %%
CACHE_DIR = Path("cache_offset_identity_smetrics_mprecision_l2")
...
def load_all_data(module_index, faultdata_dir: str = "faultdata") -> dict:
    data_dict = {}
    for folder in module_index:
        try:
...
def build_metrics_feature_matrix(
    met: pd.DataFrame,
    exclude_cols: Optional[List[str]] = None,
    standardize: bool = True,
) -> pd.DataFrame:
    """
    met: rows=modules, cols=module metrics
    exclude_cols: metrics to exclude from kernel construction
    return: numeric feature matrix for kernel construction
    """
    if exclude_cols is None:
        exclude_cols = []

    feat = met.drop(columns=exclude_cols, errors="ignore").copy()
    feat = feat.select_dtypes(include=[np.number]).copy()

    # Drop missing-value columns and constant columns.
    feat = feat.replace([np.inf, -np.inf], np.nan)
    feat = feat.dropna(axis=1, how="any")

    nunique = feat.nunique(dropna=False)
    feat = feat.loc[:, nunique > 1].copy()

    if feat.shape[1] == 0:
        raise ValueError("No usable metric columns remained for kernel construction.")

    if standardize:
        scaler = StandardScaler()
        feat_scaled = scaler.fit_transform(feat.values)
        feat = pd.DataFrame(feat_scaled, index=feat.index, columns=feat.columns)
...
def cosine_precision(df: pd.DataFrame, jitter: float = 1e-6) -> pd.DataFrame:
    """
    df: rows=modules, cols=feature dimensions
    return: precision matrix = inv(cosine kernel + jitter * I)

    This is what should be used as l2matrix when the random effect prior is
    beta ~ N(0, K), because the quadratic penalty is beta^T K^{-1} beta / 2.
    """
...
faultdata_full = load_all_data(met.index)
...
# %%
# offset: log(KLOC) where KLOC = sum_nloc / 1000
...
# %%
# smetrics = identity
identity_key = make_cache_key("identity_gram", module_names)
p = cache_path(f"df_identity_{identity_key}")
df_identity = joblib_load_if_exists(p)

if df_identity is None:
    df_identity = identity_gram(module_names)
    joblib_dump(df_identity, p)

smetdat = SMetricsData.from_dataframe(df_identity, use_index_as_name=True)


# %%
# Metric features for the kernel.
# Exclude `sum_nloc` because it is used in the offset.
metrics_features_key = make_cache_key(
    "metrics_features",
    list(met.index),
    list(met.columns),
    "exclude=sum_nloc",
...
p = cache_path(f"metrics_features_{metrics_features_key}")
metrics_features = joblib_load_if_exists(p)

if metrics_features is None:
    metrics_features = build_metrics_feature_matrix(
        met,
        exclude_cols=["sum_nloc"],
        standardize=True,
    )
    metrics_features = metrics_features.loc[module_names]
    joblib_dump(metrics_features, p)


# %%
# l2matrix = inverse(cosine kernel on metrics + jitter I)
kernel_name = "metrics_cosine"
kernel_jitter = 1e-6

kernel_key = make_cache_key(
    "l2matrix_precision",
    kernel_name,
    kernel_jitter,
    list(metrics_features.index),
    list(metrics_features.columns),
    metrics_features.shape,
)
p = cache_path(f"df_l2matrix_{kernel_key}")
df_l2matrix = joblib_load_if_exists(p)

if df_l2matrix is None:
    df_l2matrix = cosine_precision(metrics_features, jitter=kernel_jitter)
    df_l2matrix = df_l2matrix.loc[module_names, module_names]
    joblib_dump(df_l2matrix, p)

print("modules:", len(target_modules))
print("metrics raw dim:", met.shape[1])
print("metrics kernel feature dim:", metrics_features.shape[1])
print("smetrics dim:", df_identity.shape)
print("l2matrix dim:", df_l2matrix.shape)
print("l2matrix kernel:", kernel_name)
print("l2matrix jitter:", kernel_jitter)


# %%
# baseline per step (cache)
baseline_results = []
...
for i in range(len(t_end) - 1):
    ...
    step_key = make_cache_key(
        ...
        "smetrics=identity",
        f"l2matrix={kernel_name}_precision",
        f"jitter={kernel_jitter}",
    )
